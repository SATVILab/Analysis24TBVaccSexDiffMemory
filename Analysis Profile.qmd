...
library(stringr)
pca_data <- pca_data |>
  mutate(vaccine = toupper(vaccine),  # Capitalize vaccine names
         sex = str_to_title(sex)      # Capitalize the first letter of sex values
  ) |>
  rename(Vaccine = vaccine, Sex = sex)
...
```{r}
# Function to plot residuals for a given cytokine combination
plot_residuals <- function(cyt_combn_label, file_name) {
  # Filter data for the current cytokine combination and infected individuals
  data <- data_tidy_vacc_profile |> 
    dplyr::filter(ptid %in% participant_ids) |>
    dplyr::filter(cyt_combn == cyt_combn_label, infxn == "infected")
  
  # Fit the beta regression model
  full_model <- betareg(response_proportion ~ sex * vaccine, data = data)
...
# Plot residuals for "g+2+t+" combination
plot_residuals("g+2+t+", "residuals_g+2+t+.png")
...
p_inf <- ggplot(
  ...
  scale_y_continuous(limits = c(min(inf_tbl_inf$`95% CI Lower`, na.rm = TRUE), 
                                max(inf_tbl_inf$`95% CI Upper`, na.rm = TRUE) + 0.3)) +
  #scale_y_continuous(limits = c(NA, 0.6)) +
  theme(
    legend.position = "none",
    axis.title.x = element_text(size = 20),  # Increase x-axis label size
...
path_fig_inf <- "Redo Effect sizes and CI response_profile_memory1.pdf"
ggsave(
  path_fig_inf,
  p_inf,
  width = 10,
...
path_fig_inf <- "Redo Effect sizes and CI response_profile_memory2.pdf"
ggsave(
  path_fig_inf,
  p_inf,
  width = 10,
...
# Plot residuals for "g+2+t+" combination
plot_residuals("g+2+t+", "Redo residuals_g+2+t+.png")
...
```{r}
# Function to plot Q-Q plots for residuals of a given cytokine combination
plot_residuals_qq <- function(cyt_combn_label, file_name) {
  # Filter data for the current cytokine combination and infected individuals
  data <- data_tidy_vacc_profile |> 
    dplyr::filter(ptid %in% participant_ids) |>
    dplyr::filter(cyt_combn == cyt_combn_label, infxn == "infected")
...
# Q-Q plot for "g+2+t+" combination
plot_residuals_qq("g+2+t+", "QQ_Plot_residuals_g+2+t+.png")
...
```{r}
# Function to plot residuals boxplot for a given cytokine combination using ggplot2
plot_residuals_boxplot_gg <- function(cyt_combn_label, file_name) {
  # Filter data for the current cytokine combination and infected individuals
  data <- data_tidy_vacc_profile |> 
    dplyr::filter(ptid %in% participant_ids) |>
    dplyr::filter(cyt_combn == cyt_combn_label, infxn == "infected")
  ...
  # Add residuals to the data for plotting
  data$residuals <- full_model$residuals
  
  # Plot the residuals as a boxplot using ggplot2
  residual_plot <- ggplot(data, aes(x = vaccine, y = residuals, fill = vaccine)) +
    geom_boxplot() +
  ...
  # Save the plot
  ggsave(file_name, plot = residual_plot)
}

# Plot boxplot for "g+2+t+" combination
plot_residuals_boxplot_gg("g+2+t+", "Boxplot_residuals_g+2+t+.png")
