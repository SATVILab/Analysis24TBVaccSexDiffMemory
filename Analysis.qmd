...
wald_infected <- car::linearHypothesis(full_model_infected,
 c("vaccineh56 = 0", "vaccinemva85a = 0", "sexmale= 0", "vaccineh56:sexmale=0", "vaccinemva85a:sexmale= 0"))

# Likelihood ratio test:
likelihood_infected <- lmtest::lrtest(full_model_infected, null_model_infected)

# p-values from the wald and likelihood test infected
p_vec_infected <- c(wald_infected$"Pr(>F)"[2], likelihood_infected$"Pr(>Chi)"[2])
```
...
# Likelihood ratio test:
likelihood_ratio_uninfected <- lmtest::lrtest(full_model_uninfected, null_model_uninfected)

# p-values from the wald and likelihood test uninfected
p_values_simHypo_uninfected <- c()
```

## Response Profile
```{r}
cyt_data <- data_tidy_vacc_profile |> 
  select(cyt_combn)
unique_cyt<- cyt_data |>
  distinct()
num_unique <- nrow(unique_cyt) # There are 7 cyt_combn
```

```{r}
data_tidy_vacc_profile <- data_tidy_vacc_profile |>
  mutate(cyt_assign = as.numeric(as.factor(data_tidy_vacc_profile$cyt_combn)))

data_tidy_vacc_profile <- data_tidy_vacc_profile |>
   mutate(vaccine_assign = as.numeric(as.factor(data_tidy_vacc_profile$vaccine)))


pca_results <- prcomp(data_tidy_vacc_profile[,c(10,11)], scale.= TRUE)

png("pca biplot.png")
biplot(pca_results, data = data_tidy_vacc_profile,
   colour = 'vaccine_assign', label = TRUE, frame = TRUE)
dev.off()  
```